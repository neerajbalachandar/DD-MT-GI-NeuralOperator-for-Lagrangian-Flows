...
SEED = int(os.environ.get("FINAL2_GINO_SEED", "42"))
random.seed(SEED)
np.random.seed(SEED)
...
TASK = os.environ.get("FINAL2_GINO_TASK", "particle_ugradu").strip().lower()
FIELD_DECODER = os.environ.get("FINAL2_GINO_FIELD_DECODER", "super_resolution").strip().lower()
if TASK not in {"particle_ugradu", "field_reconstruction"}:
    raise ValueError("FINAL2_GINO_TASK must be particle_ugradu or field_reconstruction")
if FIELD_DECODER not in {"super_resolution", "pointwise"}:
    raise ValueError("FINAL2_GINO_FIELD_DECODER must be super_resolution or pointwise")
...
CFG = {
    ...
    "file_tag": os.environ.get(
        "FINAL2_GINO_FILE_TAG",
        "task1_particle_ugradu_gino" if TASK == "particle_ugradu" else f"task2_gino_{FIELD_DECODER}",
    ),
    "epochs": int(os.environ.get("FINAL2_GINO_EPOCHS", "60")),
    "lr": float(os.environ.get("FINAL2_GINO_LR", "3e-4")),
    "weight_decay": float(os.environ.get("FINAL2_GINO_WEIGHT_DECAY", "1e-4")),
    "eval_every": int(os.environ.get("FINAL2_GINO_EVAL_EVERY", "5")),
    "gradient_accumulation_steps": int(os.environ.get("FINAL2_GINO_ACCUM_STEPS", "4")),
    "grad_clip_norm": float(os.environ.get("FINAL2_GINO_GRAD_CLIP", "1.0")),
    "maximum_input_particles": _int_or_none(os.environ.get("FINAL2_GINO_MAX_INPUT_PARTICLES", "2000")),
    "maximum_train_output_points": _int_or_none(os.environ.get("FINAL2_GINO_MAX_TRAIN_OUTPUT_POINTS", "2048")),
    "maximum_eval_output_points": _int_or_none(os.environ.get("FINAL2_GINO_MAX_EVAL_OUTPUT_POINTS", "32768")),
    "batch_size": 1,
    "num_workers": int(os.environ.get("FINAL2_GINO_NUM_WORKERS", "0")),
    "use_amp": _bool_env("FINAL2_GINO_USE_AMP", False),
    "latent_res": int(os.environ.get("FINAL2_GINO_LATENT_RES", "8")),
    "in_gno_radius": float(os.environ.get("FINAL2_GINO_IN_RADIUS", "0.10")),
    "out_gno_radius": float(os.environ.get("FINAL2_GINO_OUT_RADIUS", "0.12")),
    "in_gno_transform_type": os.environ.get("FINAL2_GINO_IN_TRANSFORM", "nonlinear_kernelonly"),
    "out_gno_transform_type": os.environ.get("FINAL2_GINO_OUT_TRANSFORM", "linear"),
    "gno_embed_channels": int(os.environ.get("FINAL2_GINO_EMBED_CHANNELS", "24")),
    "fno_n_modes": tuple(int(x) for x in os.environ.get("FINAL2_GINO_FNO_MODES", "4,4,4").split(",")),
    "fno_hidden_channels": int(os.environ.get("FINAL2_GINO_FNO_HIDDEN", "24")),
    "fno_n_layers": int(os.environ.get("FINAL2_GINO_FNO_LAYERS", "3")),
    "projection_channel_ratio": int(os.environ.get("FINAL2_GINO_PROJ_RATIO", "2")),
    "gno_use_open3d": _bool_env("FINAL2_GINO_USE_OPEN3D", False),
    "gno_use_torch_scatter": _bool_env("FINAL2_GINO_USE_TORCH_SCATTER", False),
    "pointwise_hidden": int(os.environ.get("FINAL2_GINO_POINTWISE_HIDDEN", "96")),
    "pointwise_layers": int(os.environ.get("FINAL2_GINO_POINTWISE_LAYERS", "3")),
}


def _find_task2_dataset() -> Path:
    env = os.environ.get("FINAL2_TASK2_DATASET", "").strip()
    candidates = [Path(env)] if env else []
    candidates += [
        FINAL_DIR / "processed_data_task2" / "task2_gino_dataset.npz",
        FINAL_DIR / "processed_data" / "task2" / "task2_gino_dataset.npz",
        FINAL_DIR / "output" / "task2_gino_dataset.npz",
    ]
    for candidate in candidates:
        if candidate.exists():
            return candidate.resolve()
    raise FileNotFoundError("Could not find task2_gino_dataset.npz")
...
def _find_particle_dataset() -> Path:
    ...
    candidates = [Path(env)] if env else []
    candidates += [
        FINAL_DIR / "processed_data_task1" / "particle_ugradu_dataset.npz",
        FINAL_DIR / "output" / "particle_ugradu_dataset.npz",
    ...
    raise FileNotFoundError(
        "Could not find particle_ugradu_dataset.npz. Run final-2/preprocess_data_task1.py "
        "with TASK1_TARGET_MODE='ugradu' or set FINAL2_TASK1_UGRADU_DATASET."
    )
...
class ParticleUGradUDataset(BaseGraphDataset):
    ...
    def __getitem__(self, idx: int) -> Dict:
        ...
        input_geom = normalize_xyz(features_all[in_idx, coord_feature_indices])
        output_queries = normalize_xyz(features_all[query_idx, coord_feature_indices])
