...
class NormalizationStats:
    ...
    def fit_train_sequences(cls, data, pair_ids, feature_indices):
        ...
        next_key = "targets_next_state" if "targets_next_state" in data else "targets_residual"
        next_states = np.asarray(data[next_key], dtype=np.float32)[rows]
        return cls(x.mean(0), np.maximum(x.std(0), 1e-8), residual_rows.mean(0),
                   ...
                   np.maximum(np.ptp(combined, axis=0), 1e-8), next_states.mean(0)[:7],
                   np.maximum(next_states.std(0)[:7], 1e-8))
    ...
    def denormalize_residual(self, value):
        mean = self._tensor(self.residual_mean, value)
        std = self._tensor(self.residual_std, value)
