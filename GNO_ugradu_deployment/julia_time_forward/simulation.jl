...
const SIM_THIS_DIR = @__DIR__

run_name        = "wing-example"            # Name of this simulation

save_path       = joinpath(SIM_THIS_DIR, "output", run_name) # Where to save this simulation
paraview        = false                     # Whether to visualize with Paraview


# ----------------- SIMULATION PARAMETERS --------------------------------------
AOA             = 25.0                       # (deg) angle of attack
magVinf         = 10                      # (m/s) freestream velocity
...
function build_task1_simulation(; output_dir::String=save_path, run_label::String=run_name)
mkpath(output_dir)
...
return simulation, run_kwargs
end

if abspath(PROGRAM_FILE) == @__FILE__
    println("Running simulation...")
    simulation, run_kwargs = build_task1_simulation()
    nsteps_direct = pop!(run_kwargs, :nsteps)
    uns.run_simulation(simulation, nsteps_direct; run_kwargs...)
end
...
# # ----------------- 6) VISUALIZATION -------------------------------------------
# if abspath(PROGRAM_FILE) == @__FILE__ && paraview
