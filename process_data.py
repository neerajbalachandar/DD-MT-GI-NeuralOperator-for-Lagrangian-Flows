...
# ========== Case discovery / split configuration ==========
...
# ========== State / target / field channel names ==========
...
# ========== Composed particle input features ==========
...
# ========== Dual split (train-internal validation) ==========
...
# ========== Field grid HDF5 loader (velocity + gradients) ==========
...
# ========== Case metadata load / resolve / prepare ==========
...
# ========== Split assignment from case names ==========
...
# ========== VTK geometry loading (pyvista + fallback parser) ==========
...
# ========== Per-particle geometry features ==========
...
# ========== Train-internal validation split ==========
...
# ========== Particle payload merge (dynamic + static) ==========
INPUT_KEYS = {
    "particle_xyz": "X",
    "Gamma_vec": "Gamma",
    "velocity": "velocity",
...
# ========== Merge raw frames into per-frame NPZ ==========
...
# ========== State matrix / channel normalization / split label ==========
...
def build_particle_evolution_dataset(merged: List[Path]) -> Path:
    ...
    # ---- Build consecutive-pair rows per case ----
    for case in sorted(case_frames.keys()):
        ...
        rollout_true_states.append(np.stack(true_seq, axis=0).astype(np.float32))
    ...
    # ---- Channel normalization (train-only statistics) ----
    ...
    # ---- Coordinate normalization extents ----
